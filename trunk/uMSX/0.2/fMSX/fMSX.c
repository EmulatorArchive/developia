...
static const char *Options[]=
{
    ...
    "home","simbdos","wd1793","sound","nosound","trap","shm","noshm",
    "saver","nosaver","scale","sync","nosync","static","nostatic",
    "vsync","240","200","tv","notv","soft","nosoft",
    0
};
...
extern int   UseEffects; /* EFF_* bits, ORed (MSDOS & UNIX)     */
        ...
        else
        {
            ...
            case 21:
                ...
                if (N>=argc)
                    ...
                    {
                    case 0:
                        SETJOYTYPE(0,atoi(argv[N])&0x03);
                        break;
                    case 1:
                        SETJOYTYPE(1,atoi(argv[N])&0x03);
                        break;
                    default:
                        printf("%s: Excessive -joy option\n",argv[0]);
                    }
                break;
            case 22:
                N++;
...
#ifdef UNIX
            case 28:
                UseEffects|=EFF_MITSHM;
                break;
            case 29:
                UseEffects&=~EFF_MITSHM;
                break;
            case 30:
                UseEffects|=EFF_SAVECPU;
                break;
            case 31:
                UseEffects&=~EFF_SAVECPU;
                break;
...
//         case 37: SyncFreq=-1;break;
            ...
            case 40:
                UseEffects|=EFF_TVLINES|EFF_SCALE;
                break;
            case 41:
                UseEffects&=~(EFF_TVLINES|EFF_SCALE);
                break;
            case 42:
                UseEffects|=EFF_SOFTEN;
                break;
            case 43:
                UseEffects&=~EFF_SOFTEN;
                break;
#endif
            default:
                printf("%s: Wrong option '%s'\n",argv[0],argv[N]);
            }
        }

    /* Terminate disk lists and set initial disk names */
    if (DiskCount[0]) {
        Disks[0][DiskCount[0]]=0;
        DSKName[0]=Disks[0][0];
    }
    if (DiskCount[1]) {
        Disks[1][DiskCount[1]]=0;
        DSKName[1]=Disks[1][0];
    }
