...
void PatchZ80(Z80 *R)
{
    ...
    {
        int Sectors;
        byte Heads,Names,PerTrack,PerFAT,PerCluster;
    }
    Info[8] =
    {
        {  720,1,112,9,2,2 },
        ...
        ****************************************************************/
    {
        ...
        {
            R->AF.W=0x0201;    /* No disk      -> "Not ready"        */
            return;
        }
        if ((int)(R->DE.W)+Count>Info[R->BC.B.l-0xF8].Sectors)
        {
            R->AF.W=0x0801;    /* Wrong sector -> "Record not found" */
            return;
        }
        ...
        ****************************************************************/
    {
        ...
        if (!R->AF.B.h||(R->AF.B.h>2)) {
            R->AF.W=0x0C01;
            return;
        }
        /* If no disk, return "Not ready": */
        if (!DiskPresent(R->DE.B.h)) {
            R->AF.W=0x0201;
            return;
        }
        ...
        N=2-R->AF.B.h;
        memcpy(P,"fMSXdisk",8);
        P+=10;    /* Manufacturer's ID   */
        *P=Info[N].PerCluster;
        P+=4;      /* Sectors per cluster */
        *P++=Info[N].Names;
        *P++=0x00;    /* Number of names     */
        *P++=Info[N].Sectors&0xFF;       /* Number of sectors   */
        *P++=(Info[N].Sectors>>8)&0xFF;
        *P++=N+0xF8;                     /* Format ID [F8h-FFh] */
        *P++=Info[N].PerFAT;
        *P++=0x00;   /* Sectors per FAT     */
        *P++=Info[N].PerTrack;
        *P++=0x00; /* Sectors per track   */
        *P++=Info[N].Heads;
        *P=0x00;      /* Number of heads     */

        /* If can't write bootblock, return "Write protected": */
        if (!DiskWrite(R->DE.B.h,BootBlock,0)) {
            R->AF.W=0x0001;
            return;
        };
        ...
        for (Sector=1,J=0;J<2;J++)
        {
            ...
            if (!DiskWrite(R->DE.B.h,Buf,Sector++)) {
                R->AF.W=0x0A01;
                return;
            }
