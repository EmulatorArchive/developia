...
void MenuMSX(void)
{
    ...
    for (J=1;J;)
    {
        ...
        case 1: /* Load cartridge, disk image, state, or font */
            /* Request file name */
            P=CONFile(CLR_TEXT,CLR_BACK3,".rom\0.rom.gz\0.dsk\0.dsk.gz\0.sta\0.sta.gz\0.fnt\0.fnt.gz\0");
            /* Try loading file, show error on failure */
            if (P&&!LoadState(P)&&!LoadFile(P))
        ...
        case 4: /* Hardware model */
            for (K=1;K;)
            {
                ...
                {
                case 1:
                    if (!MODEL(MSX_MSX1))  ResetMSX((Mode&~MSX_MODEL)|MSX_MSX1,RAMPages,VRAMPages);
                    break;
                case 2:
                    if (!MODEL(MSX_MSX2))  ResetMSX((Mode&~MSX_MODEL)|MSX_MSX2,RAMPages,VRAMPages);
                    break;
                case 3:
                    if (!MODEL(MSX_MSX2P)) ResetMSX((Mode&~MSX_MODEL)|MSX_MSX2P,RAMPages,VRAMPages);
                    break;
                case 5:
                    if (!VIDEO(MSX_NTSC))  ResetMSX((Mode&~MSX_VIDEO)|MSX_NTSC,RAMPages,VRAMPages);
                    break;
                case 6:
                    if (!VIDEO(MSX_PAL))   ResetMSX((Mode&~MSX_VIDEO)|MSX_PAL,RAMPages,VRAMPages);
                    break;
                case 8:
                    ResetMSX(Mode,RAMPages<32? RAMPages*2:4,VRAMPages);
                    break;
                case 9:
                    ResetMSX(Mode,RAMPages,VRAMPages<32? VRAMPages*2:2);
                    break;
                case 11:
                    K=0;
                    break;
        ...
        case 5: /* Input devices */
            for (K=1;K;)
            {
                ...
                {
                case 1:
                    SETJOYTYPE(0,JOY_NONE);
                    break;
                case 2:
                    SETJOYTYPE(0,JOY_STICK);
                    break;
                case 3:
                    SETJOYTYPE(0,JOY_MOUSTICK);
                    break;
                case 4:
                    SETJOYTYPE(0,JOY_MOUSE);
                    break;
                case 6:
                    SETJOYTYPE(1,JOY_NONE);
                    break;
                case 7:
                    SETJOYTYPE(1,JOY_STICK);
                    break;
                case 8:
                    SETJOYTYPE(1,JOY_MOUSTICK);
                    break;
                case 9:
                    SETJOYTYPE(1,JOY_MOUSE);
                    break;
                case 11:
                    Mode^=MSX_AUTOSPACE;
                    break;
        ...
        case 6: /* Cartridge slots */
            ...
            for (K=1;K;)
            {
                ...
                case 1:
                    /* Request file name */
                    P=CONFile(CLR_TEXT,CLR_BACK3,".rom\0.rom.gz\0");
                    /* Try loading file, show error on failure */
                    ...
                    break;
                case 2:
                    LoadCart(0,N,ROMGUESS(N)|ROMTYPE(N));
                    K=0;
                    break;
                case 3:
                    Mode^=MSX_GUESSA<<N;
