...
extern char game_image_file[255];
...
char *GetRomSYSName(char *romname);
...
int ReInitMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
//   /*** Joystick types: ***/
...
///Uguru TODO: repasar aqui
//     if(emuconfig.AutoLoadState)
//     {
// LoadState(GetGameTool("ca0"));
//     }
//     if(emuconfig.AutoLoadCheat)
//     {
// if(LoadCheat(GetGameTool("cht")))puts("CheatOKI");
//     }
//   }
}
...
//     if(WorkDir=getcwd(0,1024)) Chunks[NChunks++]=WorkDir;
    ...
    /* Try loading CMOS memory contents */
    if (LoadROM(GetRomSYSName("CMOS.ROM"),sizeof(RTC),(byte *)RTC))
    {
        if (Verbose) printf("CMOS.ROM..");
    }
    else memcpy(RTC,RTCInit,sizeof(RTC));
    ...
    /* Try loading Kanji alphabet ROM */
    if (Kanji=LoadROM(GetRomSYSName("KANJI.ROM"),0x20000,0))
    {
        if (Verbose) printf("KANJI.ROM..");
    }

    /* Try loading RS232 support ROM to slot */
    if (P=LoadROM(GetRomSYSName("RS232.ROM"),0x4000,0))
    {
        if (Verbose) printf("RS232.ROM..");
        MemMap[3][3][2]=P;
        MemMap[3][3][3]=P+0x2000;
    ...
    /* ...try loading MSXDOS2 cartridge into 3:0 */
    if (!MODEL(MSX_MSX1)&&(MemMap[3][1][2]!=EmptyRAM)&&!ROMData[2])
        if (LoadCart(GetRomSYSName("MSXDOS2.ROM"),2,MAP_GEN16))
            SetMegaROM(2,0,1,ROMMask[J]-1,ROMMask[J]);
///Uguru :TODO Mirar esto
#ifdef NON_LIGERO
    /* If MSX2 or better, load PAINTER cartridge */
    if (!MODEL(MSX_MSX1))
    {
        for (;(J<MAXSLOTS)&&ROMData[J];++J);
        if ((J<MAXSLOTS)&&LoadCart(GetRomSYSName("PAINTER.ROM"),J,0)) ++J;
    }

    /* Load FMPAC cartridge */
    for (;(J<MAXSLOTS)&&ROMData[J];++J);
    if ((J<MAXSLOTS)&&LoadCart(GetRomSYSName("FMPAC.ROM"),J,MAP_FMPAC)) ++J;

    /* Load Konami GameMaster2/GameMaster cartridges */
    for (;(J<MAXSLOTS)&&ROMData[J];++J);
    if (J<MAXSLOTS)
    {
        if (LoadCart(GetRomSYSName("GMASTER2.ROM"),J,MAP_GMASTER2)) ++J;
        else if (LoadCart(GetRomSYSName("GMASTER.ROM"),J,0)) ++J;
    }
#endif
    /* We are now back to working directory */
//   if(WorkDir) chdir(WorkDir);
...
///Uguru: forzamos a que coja un joystick
...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
    ...
    if ((Mode^NewMode)&MSX_MODEL)
    {
        ...
        case MSX_MSX1:
            ...
            P1=LoadROM(GetRomSYSName("MSX.ROM"),0x8000,0);
            PRINTRESULT(P1);
        ...
        case MSX_MSX2:
            ...
            P1=LoadROM(GetRomSYSName("MSX2.ROM"),0x8000,0);
            PRINTRESULT(P1);
            if (Verbose) printf("  Opening MSX2EXT.ROM...");
            P2=LoadROM(GetRomSYSName("MSX2EXT.ROM"),0x4000,0);
            PRINTRESULT(P2);
            if (!P1||!P2)
            {
        ...
        case MSX_MSX2P:
            ...
            P1=LoadROM(GetRomSYSName("MSX2P.ROM"),0x8000,0);
            PRINTRESULT(P1);
            if (Verbose) printf("  Opening MSX2PEXT.ROM...");
            P2=LoadROM(GetRomSYSName("MSX2PEXT.ROM"),0x4000,0);
            PRINTRESULT(P2);
            if (!P1||!P2)
            {
                NewMode=(NewMode&~MSX_MODEL)|(Mode&MSX_MODEL);
...
//     if(WorkDir) chdir(WorkDir);
    ...
    if ((Mode^NewMode)&MSX_PATCHBDOS)
    {
        ...
        P1=LoadROM(GetRomSYSName("DISK.ROM"),0x4000,0);
        PRINTRESULT(P1);
...
byte InZ80(word Port)
{
    ...
    case 0xC0:
        if (Use8950) return ReadAUDIO(0);
        else return(NORAM);
    case 0xC1:
        if (Use8950) return ReadAUDIO(1);
        else return(NORAM);
#endif

        return(0xFD);                   /* Printer READY signal */
    case 0xB5:
        return(RTCIn(RTCReg));          /* RTC registers        */
...
void VDPOut(register byte R,register byte V)
{
    ...
    case  2:
        ...
        break;
    case  3:
        ColTab  = VRAM+((int)(V&MSK[ScrMode].R3)<<6)+((int)VDP[10]<<14);
    ...
    case  4:
        ...
        break;
    case  5:
        SprTab  = VRAM+((int)(V&MSK[ScrMode].R5)<<7)+((int)VDP[11]<<15);
...
// SDL_setFramerate(&fpsm,emuconfig.FPSrate);
...
int LoadFile(const char *FileName)
{
    ...
    if (!stricmpn(T,".DSK",4)||!stricmpn(T,".FDI",4))
    {
        ...
        /* Done */
        ReInitMSX(Mode,RAMPages,VRAMPages);
        return(1);
    }

    /* Try loading as a cartridge */
    if (!stricmpn(T,".ROM",4)) return(!!LoadCart(FileName,0,ROMGUESS(0)|ROMTYPE(0)));
    if (!stricmpn(T,".ZIP",4)) return(!!LoadCart(FileName,0,ROMGUESS(0)|ROMTYPE(0)));
...
byte *LoadROM(const char *Name,int Size,byte *Buf)
{
    ...
    if (check_zip(Name))
    {
        ...
        int i;
        unsigned char buf[1024], zname[256];
        memset (buf, 0, 1024);
        ...
        while (i == UNZ_OK)
        {
            unzGetCurrentFileInfo (ZF, &zinf, (char*)zname, 128, NULL, 0, NULL, 0);

            if (is_ext_zx((char*)zname, ".rom")||
                    is_ext_zx((char*)zname, ".ROM"))
            {
                i = 0;
                break;
            }
        ...
        if (i)
            return 0;
// printf("Found in ZIP %s: Size:%d \n",zname, zinf.uncompressed_size*8/0x2000);
        if (unzLocateFile (ZF, (const char *)zname, 1) != UNZ_OK)
            return 0;
...
int LoadCart(const char *FileName,int Slot,int Type)
{
    ...
    if (Verbose) printf("Found %s: Size:%d \n",FileName, Len*8/0x2000);
...
//             if (P) strcpy(P,".sta");
//             else strcat((char *)STAName,".sta");
//             /* Try loading state */
//             if (Verbose) printf("Loading state from %s...",STAName);
//             C1=LoadState(STAName);
//             PRINTRESULT(C1);
//         }
//     }
...
//             return 0;
// Size = zinf.uncompressed_size;
// //         unzReadCurrentFile (zf, &Infos, sizeof( Infos ));
// //         LongFic =unzReadCurrentFile (zf, ImgDsk, sizeof( ImgDsk));
//
//         /* Allocate memory */
//         P=Buf? Buf:GetMemory(Size);
//         if (!P)
...
//             if (strstr(currentZipFileName, ".rom") == NULL &&
//                     strstr(currentZipFileName, ".ROM") == NULL)
//             {
//                 //Last one?
...
// //       ROM64 = (int*)calloc(Len, 1);
//                 if (Verbose) printf("Found %s: Size:%d \n",currentZipFileName, Len*8/0x2000);
//
//                 /* Compute size in 8kB pages */
//                 Len>>=13;
//                 Pages=Len/0x2000;
...
//                 /* Read two bytes */
//                 C1=C2=0;
//                 unzReadCurrentFile(zip,&C1,1);
//                 unzReadCurrentFile(zip,&C2,1);
//
//                 if (Verbose) printf("  Cartridge %c: ",'A'+Slot);
//                 /* Show ROM type and size */
// //   if(Verbose)
...
//                 (
//                     "%dkB %s ROM..",Len*8,
//                     ROM64||((Len/0x2000)<=4)? "NORMAL":Type>=MAP_GUESS? "UNKNOWN":ROMNames[Type]
//                 );
//
//                 /* Assign ROMMask for MegaROMs */
//                 ROMMask[Slot]=!ROM64&&(Len>4)? (Pages-1):0x00;
...
//
//                 //Read the File
//                 if (unzReadCurrentFile(zip, ROMData[Slot], Len) >= 0)
//                 {
//                     //Load complete
...
//     }
// puts("oki");
// ///Uguru TODO: terminar la implantación de zips
//     /* Try loading ROM */
//     if (!LoadROM(FileName,Len<<13,ROMData[Slot])) {
//         PRINTFAILED;
//         return(0);
...
char *GetRomSYSName(char *romname)
{
    static char rom_filename[255]="";

    strcpy(rom_filename, emu_path);
    strcat(rom_filename, "/rom/");
    strcat(rom_filename, romname);
//     sprintf(strrchr(rom_filename, '.'), ".%s", romname);
